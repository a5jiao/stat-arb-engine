...
def backtest_pair(pair_name, db_path='data/pairs_database.db'):
    ...
    for date, row in df.iterrows():
        ...
        # *** CHANGE 2: New directional exit logic based on z-score crossing zero ***
        # Check for exit signals first
        if current_position is not None:
            should_exit = False
            exit_reason = None
            
            # Check for mean reversion exit based on direction
            if current_position['direction'] == 'long':
                # Long position: entered on negative z-score, exit when z-score goes positive (≥ 0)
                if row['z_residual'] >= 0:
                    should_exit = True
                    exit_reason = 'mean_revert'
            elif current_position['direction'] == 'short':
                # Short position: entered on positive z-score, exit when z-score goes negative (≤ 0)
                if row['z_residual'] <= 0:
                    should_exit = True
                    exit_reason = 'mean_revert'
            
            # Check for earnings exit (if enabled)
            # if signal_row['exit_earnings']:
            #     should_exit = True
    ...
    # *** CHANGE 3: Removed forced exit at end of data - let positions remain floating ***
    # if current_position is not None:
    #     final_row = df.iloc[-1]
...
def run_full_backtest(db_path='data/pairs_database.db', start_pair=0, max_pairs=None, sample_pairs=None):
    ...
    print(f"  Exit z-score threshold: Cross through 0 (directional)")  
    print(f"  Minimum implied return: {MIN_IMPLIED_RETURN:.1%}")
    print(f"  Minimum R²: {RSQ_THRESHOLD:.1%}") 
    print(f"  Maximum cointegration p-value: {COINT_THRESHOLD:.3%}")
...
if __name__ == "__main__":
    ...
    # Export results
    export_to_excel(trades, 'normal_27k_trades.xlsx')
